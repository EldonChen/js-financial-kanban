...
import { usePythonService } from '~/composables/useApi'
import { handleApiError } from '~/composables/useApiError'
import { toast } from 'vue-sonner'

const pythonService = usePythonService()
...
async function loadItems() {
  loading.value = true
  try {
    items.value = await pythonService.getItems()
    applyFilter()
  }
  catch (error) {
...
async function createItem() {
  ...
  creating.value = true
  try {
    await pythonService.createItem(formData.value)
    toast.success('创建成功')
...
async function updateItem() {
  ...
  updating.value = true
  try {
    await pythonService.updateItem(editingItem.value.id, formData.value)
    toast.success('更新成功')
...
async function confirmDelete() {
  ...
  try {
    await pythonService.deleteItem(deletingItem.value.id)
    toast.success('删除成功')
