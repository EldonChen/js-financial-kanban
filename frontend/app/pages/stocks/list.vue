...
function getQueryFromUrl() {
  ...
  return {
    ...
    market_type: (query.market_type as string) || '',
    sector: (query.sector as string) || '',
    sortBy: (query.sortBy as 'last_updated' | 'market_cap' | null) || null,
    sortOrder: (query.sortOrder as 'asc' | 'desc') || 'desc',
  }
}

// 更新 URL 参数（仅在用户操作时调用，避免循环）
...
function updateUrlParams() {
  ...
  if (selectedMarketType.value && selectedMarketType.value !== 'all') {
    query.market_type = selectedMarketType.value
  }
  ...
  const currentQueryString = JSON.stringify(
    Object.keys(currentQuery).reduce((acc, key) => {
      if (currentQuery[key]) {
...
// 搜索和筛选（从 URL 初始化）
const searchQuery = ref(urlParams.ticker || urlParams.name || '')
const selectedMarket = ref<string>(urlParams.market || '')
const selectedMarketType = ref<string>(urlParams.market_type || '')
const selectedSector = ref<string>(urlParams.sector || '')

// 排序（从 URL 初始化）
const sortBy = ref<'last_updated' | 'market_cap' | null>(urlParams.sortBy)
const sortOrder = ref<'asc' | 'desc'>(urlParams.sortOrder)
...
function openGotoDialog() {
  gotoPageInput.value = pagination.value.page.toString()
...
watch(() => route.query, (newQuery, oldQuery) => {
  // 如果还未初始化，跳过（初始化时已经在 onMounted 中处理）
...
<template>
  <div class="w-full flex flex-col gap-4">
    ...
    <Card>
      <!-- 表格上方的分页 -->
      <div v-if="!loading && !error && stocks.length > 0" class="border-b p-4">
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center gap-2">
            ...
            >
              <Icon name="lucide:arrow-right-to-line" class="h-4 w-4 mr-1" />
              跳转
            </Button>
          </div>
        </div>
    ...
    <!-- 表格下方的分页 -->
    <div v-if="!loading && !error && stocks.length > 0" class="border-t p-4">
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-2">
          <span class="text-sm text-muted-foreground">每页显示：</span>
          <Select
    ...
    <!-- Goto 页面对话框 -->
    <Dialog v-model:open="showGotoDialog">
      <DialogContent>
        <DialogHeader>
          <DialogTitle>跳转到指定页面</DialogTitle>
          <DialogDescription>
            请输入要跳转的页码（1 - {{ pagination.total_pages }}）
          </DialogDescription>
        </DialogHeader>
        ...
        <DialogFooter>
          <Button variant="outline" @click="showGotoDialog = false">
            取消
          </Button>
