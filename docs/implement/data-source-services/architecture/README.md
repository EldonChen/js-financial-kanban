...
| [service-layout.md](service-layout.md) | 代码目录、分层、与 SPEC 分层的映射；含应用入口与多端扩展说明 |
...
- **入口可扩展**：应用核心为 `data_source_app_service`，REST 为默认入口；SDK、MCP 等可通过新增适配层复用同一业务逻辑与契约，无需改动 domain/router/registry/providers。
